...
1. Купите VPS в другой стране. Оплатить лучше криптовалютой, для конфеденциальности Владельца.
2. Установите образ Ubuntu 24.04 на VPS. Используйте [браузер](https://github.com/TheTorProject) [TOR browser](https://www.torproject.org/download/) для приатности подключения к Панели управления VPS и сайту хостера. Для мобильных устройств можете использовать [Orbot].(https://play.google.com/store/apps/details?id=org.torproject.android&hl=en-US)
3. Сгенерируйте новый пароль на сервере в Панели управления и скопируйте его в буфер обмена.
> Для подключения к серверу другими способами: Сгенерируйте SSH ключи на своём устройстве и пропишите PUB ключ в Панели Управления VPS. 🚫 Не рекомендую подключаться с VPS по внешнему IP, так как **это скомпроментирует Вас** и сервер. 
...
6. Нажмите Подключить (в клиенте долно быть всё понятно интуитивно). 💡 Включите опцию «Use Remote DNS» и режим «VPN» в настройках клиента!
7. Владельцам IPHONE: в консоли сервера введите команду `iosuser` и отсканировать QR код.💡 Этот профиль адаптирован под [платное](https://apps.apple.com/ru/app/shadowrocket/id932747118) приложение [SHADOWROCKET](https://shadowrocketvpn.com/) так как оно имеет встроенную **TLS библиотеку Apple** и это **фундаментальный компонент**, который превращает связку **VLESS + REALITY + Vision** из набора протоколов в мощный, целостный инструмент для обхода цензуры, обеспечивающий высочайший уровень скрытности и производительности на мобильных устройствах! 
8. Настройте пользователей и получите ссылки для них: Три вида профилей пользователей: **Android** - Chrome fingerprint + Google маскировка; **Стандартный** - Firefox fingerprint + Cloudflare маскировка; **Кастомный** - Ручной ввод SNI и fingerprint.
9. По окончании настройки пользователей введите `closessh` для закрытия всех портов, кроме 443/TCP. Доступ к VPS останется только из консоли хостера в Панели Управления VPS. ✨
