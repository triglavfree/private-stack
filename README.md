...
1. Оплатите криптовалютой VPS в другой стране.
2. Установите Ubuntu 24.04
3. Сгенерируйте новый пароль на сервере в Панели управления и скопируйте его в буфер обмена.
4. Сгенерируйте SSH ключи на своём устройстве и пропишите PUB в Панели Управления VPS (если необходимо подключение к серверу)
5. Используйте консоль панели управления VPS для подключения к серверу. логин `root` пароль `из буфера обмена`.
6. Скопируйте ссылку для запуска скрипта и вставьте её в консоль сервера.
7. Нажмите `Enter`.
8. Скопируйте ссылку с экрана по окончанию выполнения скрипта в буфер обмена и вставьте в профиль клиента на устройстве или отсканируйте QR код клиентом на Вашем устройстве.
9. Нажмите Подключить (в клиенте долно быть всё понятно интуитивно).
10. Если у Вас IPHONE, то можно ввести в консоли сервера команду `iosuser` и отсканировать QR код. (💡 Этот профиль адаптирован под платное приложение [SHADOWROCKET](https://shadowrocketvpn.com/)[App Store](https://apps.apple.com/ru/app/shadowrocket/id932747118) так как оно имеет свою **TLS** библиотеку и это **фундаментальный компонент**, который превращает связку **VLESS + REALITY + Vision** из набора протоколов в мощный, целостный инструмент для обхода цензуры, обеспечивающий высочайший уровень скрытности и производительности на мобильных устройствах!
...
closessh      # Закрыть все порты кроме 443/TCP (ufw)
