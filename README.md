...
4. Для подключения к серверу другими способами: Сгенерируйте SSH ключи на своём устройстве и пропишите PUB ключ в Панели Управления VPS. 🚫 Не рекомендую это делать, так как **это скомпроментирует Вас** и сервер. 
5. Используйте консоль панели управления VPS для подключения к серверу. логин `root` пароль `из буфера обмена`.
6. Вставьте скопированную ссылку для запуска скрипта и Нажмите `Enter`  в консоль сервера.
7. Скопируйте ссылку с экрана по окончанию выполнения скрипта в буфер обмена и вставьте в профиль клиента на устройстве или отсканируйте QR код клиентом на Вашем устройстве.
8. Нажмите Подключить (в клиенте долно быть всё понятно интуитивно). 💡 Включите опцию «Use Remote DNS» и режим «VPN» в настройках клиента!
9. Владельцам IPHONE: в консоли сервера введите команду `iosuser` и отсканировать QR код.💡 Этот профиль адаптирован под платное приложение [SHADOWROCKET](https://shadowrocketvpn.com/)[App Store](https://apps.apple.com/ru/app/shadowrocket/id932747118) так как оно имеет свою **TLS библиотеку Apple** и это **фундаментальный компонент**, который превращает связку **VLESS + REALITY + Vision** из набора протоколов в мощный, целостный инструмент для обхода цензуры, обеспечивающий высочайший уровень скрытности и производительности на мобильных устройствах! 
10. Настройте пользователей и получите ссылки для них: Три вида профилей пользователей: **Android** - Chrome fingerprint + Google маскировка; **Стандартный** - Firefox fingerprint + Cloudflare маскировка; **Кастомный** - Ручной ввод SNI и fingerprint.
11. По окончании настройки пользователей введите `closessh` для закрытия всех портов, кроме 443/TCP. Доступ к VPS останется только из консоли хостера в Панели Управления VPS. ✨
