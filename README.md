[![Linux Kernel](https://img.shields.io/badge/Linux%20Kernel-%E2%98%85%205.17%2B-blue?style=flat-square&logo=linux&labelColor=000000&color=007A33&logoColor=white)](https://github.com/torvalds/linux)
[![Hiddify-Manager](https://img.shields.io/github/v/release/hiddify/hiddify-manager?style=flat-square&logo=github&label=Hiddify-Manager&color=red&labelColor=fffffft&logoColor=ffffff)](https://github.com/hiddify/hiddify-manager/releases)
[![Project X](https://img.shields.io/badge/Project-X-000000?logo=github&logoColor=white)](https://xtls.github.io/ru/config/)
[![WireGuard Installer](https://img.shields.io/badge/WireGuard%20Installer-nyr-blue?logo=github)](https://github.com/Nyr/wireguard-install)
[![wireguard-go](https://img.shields.io/badge/WireGuard%20Go-blue?logo=github)](https://github.com/WireGuard/wireguard-go)
[![Xray-Core](https://img.shields.io/badge/Xray%20Core-25.10.15-8A2BE2)](https://github.com/XTLS/Xray-core)
[![V2Fly](https://img.shields.io/badge/V2Fly-Community--Driven%20Edition-blue?logo=github)](https://github.com/v2fly)
[![sing-box](https://img.shields.io/badge/sing--box-1.12.12-8A2BE2?logo=github&logoColor=white)](https://github.com/SagerNet/sing-box)
![VLESS](https://img.shields.io/badge/VLESS-Protocol-green)
![Reality](https://img.shields.io/badge/Reality-Transport-green)
![Vision](https://img.shields.io/badge/Vision-Encryption-blue)
![VPS](https://img.shields.io/badge/VPS-1GB_RAM-lightgrey)
![BBR](https://img.shields.io/badge/BBR-Optimized-yellow)
![IPv4 Only](https://img.shields.io/badge/IPv4-Only-red?logo=ipv4)
[![Ubuntu](https://img.shields.io/badge/Ubuntu-≥22.04%20Minimal-E95420?logo=ubuntu&logoColor=white)](https://releases.ubuntu.com/24.04/)
[![Ask DeepWiki](https://img.shields.io/badge/Ask-DeepWiki-007ACC?logo=data:image/svg+xml;base64,&logoColor=white&label=&labelColor=black)](https://deepwiki.com/XTLS/Xray-core)
[![DeepSeek AI](https://img.shields.io/badge/Chat-DeepSeek%20AI-007BFF?logo=ai&logoColor=white)](https://chat.deepseek.com)


# PRIVATE STACK 🛡️ [VLESS](https://xtls.github.io/ru/development/protocols/vless.html) + [REALITY](https://xtls.github.io/ru/config/transport.html) + [VISION](https://deepwiki.com/XTLS/Xray-examples/2.2-vless-+-tcp-+-xtls-vision) 
<div align="center">
🔐 НЕТ ЛОГОВ • 🛡️ НЕТ ТРЕКИНГА • ⚡ НЕТ КОМПРОМИССОВ

Сделано для тех, кто ценит приватность
</div>


## 🪬 КАК ИСПОЛЬЗОВАТЬ

1. Купите VPS. [пример](https://bitcoin-vps.com/)
>Оплатить лучше криптовалютой, для конфеденциальности Владельца.
2. Установите образ Ubuntu ≥ 22.04 Minimal на Ваш VPS.
> Используйте [браузер](https://github.com/TheTorProject) [TOR browser](https://www.torproject.org/download/) для приатности подключения к Панели управления VPS и сайту хостера. Для мобильных устройств можете использовать [Orbot](https://play.google.com/store/apps/details?id=org.torproject.android&hl=en-US).
3. Сгенерируйте новый пароль `root` в Панели управления и скопируйте его в буфер обмена.
> Для подключения к серверу другими способами:
> Сгенерируйте SSH ключи на своём устройстве и пропишите PUB ключ в Панели Управления VPS. Обычно по умолчанию на VPS после установки образа ОС открыты 22,80,443 порты.
> 🚫 Не рекомендую подключаться к VPS по внешнему IP, так как это **скомпроментирует Вас** и **сервер**. 
4. Используйте консоль панели управления VPS для подключения к серверу: логин `root` пароль `из буфера обмена`. Вставьте ссылку для запуска скрипта и Нажмите `Enter`  в консоль сервера.
5. Скопируйте ссылку с экрана по окончанию выполнения скрипта в буфер обмена и вставьте в профиль клиента на устройстве или отсканируйте QR код клиентом на Вашем устройстве.
6. Нажмите Подключить (в клиенте долно быть всё понятно интуитивно).
> 💡 Включите опцию **Use Remote DNS** и режим **VPN** в настройках клиента!
>
> **Владельцам IPHONE**: в консоли сервера введите команду `iosuser` и отсканируйте QR код.
>- Профиль **iosuser** оптимизирован под [платное](https://apps.apple.com/ru/app/shadowrocket/id932747118) приложение [SHADOWROCKET](https://shadowrocketvpn.com/) так как использует встроенную **TLS библиотеку Apple**. Это **фундаментальный компонент**, который превращает связку **VLESS + REALITY + Vision** из набора протоколов в мощный, целостный инструмент для обхода цензуры, обеспечивающий **Высочайший уровень скрытности и производительности** на мобильных устройствах **(VLESS/XTLS-RPRX-VISION/UDP**). Но и в остальных клиентах на **iOS/macOS** профиль работает Великолепно🏆
7. Настройте пользователей и получите ссылки для них.
>*Четыре* вида профилей пользователей:
>- **Стандартный** : Firefox fingerprint + Cloudflare маскировка;
>- **iOS** : iOS + iCloud маскировка;
>- **Android** : Chrome fingerprint + Google маскировка;
>- **Другой** : Ручной ввод SNI и fingerprint.
8. По окончании настройки пользователей используйте команду `closessh` для закрытия всех портов, кроме 443/TCP.
> Доступ к VPS останется только из консоли хостера в Панели Управления VPS.
---
## ‼️Подготовка VPS
```bash
curl -sSL https://raw.githubusercontent.com/triglavfreeprivate-stack/main/vps_setup.sh | sudo bash
```
## 🚀 ЗАПУСК СКРИПТА
###  [xray-core](https://github.com/XTLS/Xray-core) *Индивидуальный*
```bash
curl -sL https://raw.githubusercontent.com/triglavfree/private-stack/main/xray-core | bash
```
>⚠️ Не устанавливайте оба на один сервер — они конфликтуют за порт 443. 

###  [sing-box](https://github.com/SagerNet/sing-box) *Групповой*
```bash
curl -sL https://raw.githubusercontent.com/triglavfree/private-stack/main/sing-box | bash
```
## 🎯 АВТОМАТИЧЕСКАЯ НАСТРОЙКА
🔧 **Системная оптимизация**
- **BBR Turbo** - оптимизировано для 1GB RAM VPS
- **NVMe Boost** - специальные настройки для SSD дисков
- **Swap Intelligence** - автоматическое создание 2GB swap при RAM ≤ 1GB
- **System Limits** - увеличение лимитов файловых дескрипторов
  
🛡️ **Безопасность**
- **IPv4 only** - Отключает IPv6 на системном уровне
- **UFW Firewall** - только порт 443/TCP, всё остальное заблокировано
- **Zero-Logs Architecture** - логи только в RAM, очистка при перезагрузке
- **Fail2Ban Protection** - защита от брутфорс-атак SSH
- **Telemetry Blocking** - блокировка слежки Microsoft/Google
  
🌐 **Сетевая конфигураци**
- **VLESS + REALITY + Vision** - необнаруживаемый протокол
- **DNS over HTTPS** - Cloudflare DoH `https://1.1.1.1/dns-query`
- **Traffic Obfuscation** - маскировка под `www.cloudflare.com`
- **Geo-IP Filtering** - интеллектуальная маршрутизация трафика

👥 **Умное управление пользователями**
- **Два профиля по умолчанию**: main (Cloudflare) и ios (iCloud)
- **Автоопределение типа** - умный подбор параметров по имени пользователя
- **QR-коды** - мгновенное подключение мобильных устройств


## 📱 [КЛИЕНТЫ ДЛЯ ВСЕХ ПЛАТФОРМ](https://xtls.github.io/ru/document/install.html#%D0%B3%D1%80%D0%B0%D1%84%D0%B8%D1%87%D0%B5%D1%81%D0%BA%D0%B8%D0%B5-%D0%BA%D0%BB%D0%B8%D0%B5%D0%BD%D1%82%D1%8B)
| Платформа     | Клиент   | Где взять                     |
|---------------|----------|-------------------------------|
| **iOS/macOS** | Hiddify  | [GitHub Releases](https://github.com/hiddify/hiddify-app/releases)|
| **Android**   | Hiddify  | [GitHub Releases](https://github.com/hiddify/hiddify-app/releases) |
| **Windows**   | Hiddify  | [GitHub Releases](https://github.com/hiddify/hiddify-app/releases) |
| **Linux**     | Hiddify  | [GitHub Releases](https://github.com/hiddify/hiddify-app/releases) |

>💡 ВАЖНО: Включите опцию «Use Remote DNS» и режим «VPN» в настройках клиента!

## 🎮 КОМАНДЫ УПРАВЛЕНИЯ
После установки доступны команды:
```yaml
mainuser      # выводит ссылку для подключения основного пользователя (Cloudflare)
iosuser       # выводит ссылку для iOS устройства (iCloud маскировка)
githubuser    # выводит ссылку для подключения пользователя (Github)
adduser       # создает нового пользователя
listuser      # выводит список клиентов
rmuser        # удаление пользователей
status        # показывает статус всей системы
sharelink     # показывает ссылки и QR пользователя для подключения
update        # Включает автообновление ядра и geo баз
openssh       # Открывает SSH для Внешнего IP и включает защиту от брутфорса (устанавливает fail2ban)
closessh      # Закрывает все порты кроме 443/TCP (устанавливает ufw)
cat help      # Список доступных пользовательских команд
```
## 🔒 АРХИТЕКТУРА БЕЗОПАСНОСТИ

🚫 **Принцип "NO LOGS"**
- **Системные логи**: Только в RAM, максимальный размер 16MB, время хранения 30 секунд
- **Xray логи**: Уровень **warning**, доступ и ошибки отключены, статистика выключена
- **DNS логи**: Полностью отключено логирование DNS запросов

🛡️ **Защита от слежки**
```json
"hosts": {
    "geo.prod.do.dsp.mp.microsoft.com": "127.0.0.1",
    "telemetry.microsoft.com": "127.0.0.1", 
    "telemetry.google.com": "127.0.0.1",
    "google-analytics.com": "127.0.0.1"
}
```

🌐 **Умный DNS и маршрутизация**

- **DoH шифрование** - все DNS запросы через HTTPS
- **Блокировка рекламы** - встроенные списки ad-block
- **Geo-IP правила** - прямой доступ к локальным ресурсам
- **BitTorrent блокировка** - предотвращение P2P трафика

## 🎨 ОСОБЕННОСТИ ПРОФИЛЕЙ

📱 **iOS Профиль (iosuser)**
```bash
Маскировка: www.icloud.com
Fingerprint: iOS
Оптимизация: Встроенная TLS библиотека Apple
```
💻 **Основной профиль (mainuser)**
```bash
Маскировка: www.cloudflare.com  
Fingerprint: Firefox
Универсальность: Подходит для всех устройств
```
🔧 **Кастомные профили (adduser)**
- **Android** - Chrome fingerprint + Google маскировка
- **Стандартный** - Firefox fingerprint + Cloudflare маскировка
- **Github** - Firefox fingerprint + Github маскировка
- **Другой** - Ручной ввод SNI и fingerprint

---
<div align="center">
🔐 НЕТ ЛОГОВ • 🛡️ НЕТ ТРЕКИНГА • ⚡ НЕТ КОМПРОМИССОВ

Сделано для тех, кто ценит приватность
</div>
